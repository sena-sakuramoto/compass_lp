...
const AI_PROVIDER_ICON_PATHS: Record<AiProviderId, string> = {
...
公開ページを確認し、事実、推測、提案を分けて日本語で回答してください。ページに書かれている事実を優先し、そこからの推測と提案を明確に区別してください。
...
export function providerIconSvg(provider: AiProviderId): string {
  const path = AI_PROVIDER_ICON_PATHS[provider];
  const fill = provider === 'chatgpt'
    ? '#0d0d0d'
    : provider === 'claude'
      ? '#d97757'
      : provider === 'perplexity'
        ? '#20808d'
        : 'url(#ai-provider-gemini-gradient)';
  const gradient = provider === 'gemini'
    ? '<defs><linearGradient id="ai-provider-gemini-gradient" x1="2" y1="22" x2="22" y2="2" gradientUnits="userSpaceOnUse"><stop stop-color="#3186ff"/><stop offset=".52" stop-color="#7b5ce1"/><stop offset="1" stop-color="#d96570"/></linearGradient></defs>'
    : '';

  return `<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" data-ai-provider-icon="${provider}">${gradient}<path fill="${fill}" d="${path}"/></svg>`;
}
